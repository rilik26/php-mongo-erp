 ...
 * - Evraka Git: i18n/lang_admin veya gendoc_edit'e gider
 *
 * Guard:
 * - login şart
 */
...
$onlyActive = (($_GET['active'] ?? '1') !== '0'); // default active=1
...
<head>
  ...
  <style>
    ...
    .btn{ padding:6px 10px; border:1px solid #ccc; background:#fff; cursor:pointer; text-decoration:none; color:#000; border-radius:6px; display:inline-flex; align-items:center; gap:6px; }
    ...
    .muted{ color:#888; }
    input[type="text"]{ padding:6px 8px; border:1px solid #ddd; border-radius:6px; min-width:240px; }
    label{ font-size:12px; color:#444; }
    .toast-area{ margin:10px 0; }
  </style>
</head>
<body>
...
<form method="GET" class="bar" id="filterForm">
  <label>Search</label>
...
<table id="locksTable">
  ...
  <?php else: ?>
    ...
    ?>
      <tr
        data-module="<?php echo esc($tModule); ?>"
        data-doc-type="<?php echo esc($tDocType); ?>"
        data-doc-id="<?php echo esc($tDocId); ?>"
      >
        <td><?php echo badge_html($status); ?></td>

        <td class="small"><?php echo esc(fmt_tr_dt($lockedIso)); ?></td>
        ...
        <td>
          <?php if ($docUrl): ?>
            <a class="btn" href="<?php echo esc($docUrl); ?>" target="_blank">Evraka Git</a>
          <?php else: ?>
            <span class="btn" style="opacity:.45; cursor:default;">Evraka Git</span>
          <?php endif; ?>

          <button class="btn js-acquire" type="button" data-url="<?php echo esc($acqUrl); ?>">Acquire</button>

          <?php if ($isMine): ?>
            <button class="btn btn-danger js-release" type="button" data-url="<?php echo esc($relUrl); ?>">Release</button>
          <?php elseif ($isAdmin): ?>
            <button class="btn btn-danger js-release" type="button" data-url="<?php echo esc($forceUrl); ?>">Force</button>
          <?php else: ?>
...
(function(){
  ...
  function reloadSoon(){
    // filtreler kalsın diye aynı url'yi reload ediyoruz
